...
class ImageCompressPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        //set config
        project.extensions.create("ImageConfig", Config::class.java)
        mcImageConfig = project.property("ImageConfig") as Config
        ...
        project.afterEvaluate {
            variants.all { variant ->
                ...
                val mcPicTask = project.task("ImageCompress${variant.name.capitalize()}")
                mcPicTask.group = "image"
                mcPicTask.doLast {
                    //debug enable
...
//                    mtDispatchOptimizeTask(bigImgList)
    ...
    private fun sizeInfo(): String {
        return "->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n" +
                ...
                "Image optimize size: " + (oldSize - newSize) / 1024 + "KB\n" +
                "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<-"
